...
class SigmaPoints(object):
    ...
    def __init__(self, sqrt_method = None, add = None):
        ...
        self.sigma_functions['mysovskikh'] = self.__get_mysovskikh_set__
    ...
    def get_set(self, x, Px, set_name, **scale_args):
        ...
        **args : scalar scaling variables
        ...
        X = self.add(x[:,None].repeat(2*N**2 + 1, axis = 1), self.sqrt(Px)@X)
    ...
    def __get_set_mwer__(self, n, **scale_args):
       ...
       """
        ...
        ### Sigma points
        X = (n + kappa)*np.block([np.zeros(n), np.eye(n), -np.eye(n)])
    ...
    def __get_set_gauss__(n, **scale_args):
       ...
       return self.__get_set_gauss__(n, alpha = 1., beta = 0., kappa = 1.)
